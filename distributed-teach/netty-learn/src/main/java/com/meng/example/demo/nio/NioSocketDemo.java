package com.meng.example.demo.nio;
...
public class NioSocketDemo {
    ...
    public void initServer(int port) throws IOException {
        ...
        serverChannel.socket().bind(new InetSocketAddress("192.168.9.151",port));
        // 获得一个通道选择器(管理器)
        ...
        while (true) {
          ...
          //  this.selector.select(1000);
            ...
            while (iteratorKey.hasNext()) {
                ...
                // 删除已选的key,以防重复处理
                //此方法并不会真正移除selectionKey ，当handler方法执行完毕后才会真正移除selectionKey
                //所以 thread中处理handler会出问题
                iteratorKey.remove();
